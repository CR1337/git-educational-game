...
signal request_completed(status_code: int, body: Array[String])
...
func _request_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
    ...
    for fn in JSON.parse_string(body.get_string_from_utf8()):
        filenames.append(fn)

    emit_signal("request_completed", response_code, filenames)
